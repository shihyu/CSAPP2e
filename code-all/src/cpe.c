		     ...
		     sample_t smethod, double bias, int verbose)
{
	...
	if (cycle_val[i] < 1.0) {
	    fprintf(stderr, "Got %.2f cycles for count %d\n", cycle_val[i], cnt);
	}
    }
    /* Fit data */
	...
	/* Print results */
	fprintf(data_file, "cpe\t%.2f\tovhd\t%.2f\ttovhd\t%.2f\tavgerr\t\%.3f\tmaxerr\t\%.3f\n",
		cpe, overhead, timer_overhead,
