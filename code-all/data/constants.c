...
typedef enum { GCC, C89, C99 } lang_t;

char *langs[3] = {"GCC", "C89", "C99"};
...
int main(int argc, char *argv[])
{
    ...
    ll_t tmin_dec_val = (ll_t) TMIN_DEC;
    ll_t tmin_hex_val = (ll_t) TMIN_HEX;
    char * CZd = COMP_ZERO(TMIN_DEC);
    char * CZh = COMP_ZERO(TMIN_HEX);
    char * czd = comp_zero(TMIN_DEC);
    char * czh = comp_zero(TMIN_HEX);
    ll_t TWICEd = TWICE(TMIN_DEC);
    ll_t TWICEh = TWICE(TMIN_HEX);
	...
	printf("Dialect = %s\n", langs[LANG]);
	printf("    COMP_ZERO\n");
	printf("\t%s\t(0x%llx) %s 0\n", STMIN_DEC, (ll_t) TMIN_DEC, COMP_ZERO(TMIN_DEC));
	printf("\t%s\t(0x%llx) %s 0\n", STMIN_HEX, (ll_t) TMIN_HEX, COMP_ZERO(TMIN_HEX));

	printf("    COMP_TMIN\n");
	printf("\t%s\t(0x%llx) %s %s\n", STMIN_DEC, (ll_t) TMIN_DEC, COMP_TMIN(TMIN_DEC), STMIN_DEC);
	printf("\t%s\t(0x%llx) %s %s\n", STMIN_HEX, (ll_t) TMIN_HEX, COMP_TMIN(TMIN_HEX), STMIN_DEC);
	...
	printf("    TWICE\n");
	printf("\t2*%s\t(2*0x%llx) = %lld (0x%llx)\n", STMIN_DEC, (ll_t) TMIN_DEC, (ll_t) TWICE(TMIN_DEC), (ll_t) TWICE(TMIN_DEC));
	printf("\t2*%s\t(2*0x%llx) = %lld (0x%llx)\n", STMIN_HEX, (ll_t) TMIN_HEX, (ll_t) TWICE(TMIN_HEX), (ll_t) TWICE(TMIN_HEX));
